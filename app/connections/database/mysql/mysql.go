...
func init() {
	...
	if err != nil {
		...
		SqlDB.SetMaxIdleConns(config.GetEnv().MaxIdleConns) 
		SqlDB.SetMaxOpenConns(config.GetEnv().MaxOpenConns)
	...
	for k, v := range cons {
		...
		tempSql.SetMaxIdleConns(v.MaxIdleConns)
		tempSql.SetMaxOpenConns(v.MaxOpenConns)
		sqlDBmap[k] = tempSql
	}
}
